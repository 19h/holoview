    ...
    ($pass_struct:ty) => {
        impl PostPass for $pass_struct {
            fn draw(&self, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView) {
                ...
                rp.set_vertex_buffer(0, self.fsq_vb.slice(..));
                rp.draw(0..3, 0..1);
            }
        }
    };
...
pub struct EdlUniforms {
    pub inv_size: [f32; 2], pub strength: f32, pub radius_px: f32,
}
impl Default for EdlUniforms {
    fn default() -> Self { Self { inv_size: [1.0/1280.0, 1.0/720.0], strength: 1.6, radius_px: 1.5 } }
}

pub struct EdlPass {
    pub pipeline: wgpu::RenderPipeline, pub bind_group: wgpu::BindGroup, pub bgl: wgpu::BindGroupLayout,
    pub uniform: EdlUniforms, pub uniform_buffer: wgpu::Buffer, pub fsq_vb: wgpu::Buffer,
}
...
impl EdlPass {
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("EDL Uniforms"), contents: bytemuck::bytes_of(&uniform),
        ...
        let fsq_vb = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("FSQ VB"), contents: bytemuck::cast_slice(&fsq), usage: wgpu::BufferUsages::VERTEX,
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("EDL WGSL"), source: wgpu::ShaderSource::Wgsl(EDL_WGSL.into()),
        });
        let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("EDL PL"), bind_group_layouts: &[&bgl], push_constant_ranges: &[],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("EDL Pipeline"), layout: Some(&pl),
            ...
            fragment: Some(wgpu::FragmentState {
                ...
                targets: &[Some(wgpu::ColorTargetState { format: dst_format, blend: Some(wgpu::BlendState::REPLACE), write_mask: wgpu::ColorWrites::ALL })],
                compilation_options: wgpu::PipelineCompilationOptions::default(),
            }),
        ...
        let dummy = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("EDL Dummy"), size: wgpu::Extent3d { width: 1, height: 1, depth_or_array_layers: 1 }, mip_level_count: 1, sample_count: 1,
            dimension: wgpu::TextureDimension::D2, format: dst_format,
        ...
        Self { pipeline, bind_group, bgl, uniform, uniform_buffer, fsq_vb }
    }
    pub fn set_inputs(&mut self, device: &wgpu::Device, color_view: &wgpu::TextureView, depthlin_view: &wgpu::TextureView, sampler: &wgpu::Sampler) {
        self.bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("EDL BG (updated)"), layout: &self.bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(color_view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(depthlin_view) },
...
pub struct RgbShiftUniforms {
    pub inv_size: [f32; 2], pub amount: f32, pub angle: f32,
}
impl Default for RgbShiftUniforms {
    fn default() -> Self { Self { inv_size: [1.0/1280.0, 1.0/720.0], amount: 0.0016, angle: 0.0 } }
}

pub struct RgbShiftPass {
    pub pipeline: wgpu::RenderPipeline, pub bgl: wgpu::BindGroupLayout, pub bind_group: wgpu::BindGroup,
    pub uniform: RgbShiftUniforms, pub uniform_buffer: wgpu::Buffer, pub fsq_vb: wgpu::Buffer,
}
...
impl RgbShiftPass {
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("RGBShift Uniforms"), contents: bytemuck::bytes_of(&uniform),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let fsq = [[-1.0_f32, -3.0], [3.0, 1.0], [-1.0, 1.0]];
        let fsq_vb = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("RGBShift FSQ"), contents: bytemuck::cast_slice(&fsq), usage: wgpu::BufferUsages::VERTEX,
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("RGBShift WGSL"), source: wgpu::ShaderSource::Wgsl(RGBSHIFT_WGSL.into()),
        });
        let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("RGBShift PL"), bind_group_layouts: &[&bgl], push_constant_ranges: &[],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("RGBShift Pipeline"), layout: Some(&pl),
            vertex: wgpu::VertexState {
                module: &shader, entry_point: "vs_main",
                buffers: &[wgpu::VertexBufferLayout {
                    array_stride: std::mem::size_of::<[f32; 2]>() as wgpu::BufferAddress,
        ...
        let dummy = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("RGBShift Dummy"), size: wgpu::Extent3d { width: 1, height: 1, depth_or_array_layers: 1 }, mip_level_count: 1, sample_count: 1,
            dimension: wgpu::TextureDimension::D2, format: dst_format,
        ...
        let samp = device.create_sampler(&wgpu::SamplerDescriptor::default());
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("RGBShift BG"), layout: &bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&view) },
                wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::Sampler(&samp) },
        ...
        Self { pipeline, bgl, bind_group, uniform, uniform_buffer, fsq_vb }
    }
    pub fn set_input(&mut self, device: &wgpu::Device, src_view: &wgpu::TextureView, depthlin_view: &wgpu::TextureView, sampler: &wgpu::Sampler) {
        self.bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("RGBShift BG (updated)"), layout: &self.bgl,
            entries: &[
...
pub struct CrtUniforms {
    pub inv_size: [f32; 2], pub time: f32, pub intensity: f32, pub vignette: f32, pub _padding: f32,
}
impl Default for CrtUniforms {
    fn default() -> Self { Self { inv_size: [1.0/1280.0, 1.0/720.0], time: 0.0, intensity: 0.08, vignette: 0.35, _padding: 0.0 } }
}

pub struct CrtPass {
    pub pipeline: wgpu::RenderPipeline, pub bgl: wgpu::BindGroupLayout, pub bind_group: wgpu::BindGroup,
    pub uniform: CrtUniforms, pub uniform_buffer: wgpu::Buffer, pub fsq_vb: wgpu::Buffer,
}
...
impl CrtPass {
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("CRT Uniforms"), contents: bytemuck::bytes_of(&uniform),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("CRT BGL"),
        ...
        let fsq = [[-1.0f32, -3.0], [3.0, 1.0], [-1.0, 1.0]];
        let fsq_vb = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("CRT FSQ"), contents: bytemuck::cast_slice(&fsq), usage: wgpu::BufferUsages::VERTEX,
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("CRT WGSL"), source: wgpu::ShaderSource::Wgsl(CRT_WGSL.into()),
        });
        let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("CRT PL"), bind_group_layouts: &[&bgl], push_constant_ranges: &[],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("CRT Pipeline"), layout: Some(&pl),
            vertex: wgpu::VertexState {
                module: &shader, entry_point: "vs_main",
                buffers: &[wgpu::VertexBufferLayout {
                    array_stride: std::mem::size_of::<[f32;2]>() as wgpu::BufferAddress,
        ...
        let dummy = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("CRT Dummy"), size: wgpu::Extent3d { width: 1, height: 1, depth_or_array_layers: 1 }, mip_level_count:1, sample_count:1,
            dimension: wgpu::TextureDimension::D2, format: dst_format,
            usage: wgpu::TextureUsages::TEXTURE_BINDING | wgpu::TextureUsages::RENDER_ATTACHMENT, view_formats: &[],
        });
        let view = dummy.create_view(&wgpu::TextureViewDescriptor::default());
    ...
    pub fn set_inputs(&mut self, device: &wgpu::Device, color_view: &wgpu::TextureView, depthlin_view: &wgpu::TextureView, sampler: &wgpu::Sampler) {
        self.bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("CRT BG (updated)"), layout: &self.bgl,
            entries: &[
                wgpu::BindGroupEntry { binding:0, resource: wgpu::BindingResource::TextureView(color_view) },
                wgpu::BindGroupEntry { binding:1, resource: wgpu::BindingResource::TextureView(depthlin_view) },
                wgpu::BindGroupEntry { binding:2, resource: wgpu::BindingResource::Sampler(sampler) },
