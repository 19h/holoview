...
pub struct EdlUniforms {
    pub inv_size: [f32; 2], // 1/width, 1/height
    pub strength: f32,      // ~1.6
    pub radius_px: f32,     // ~1.5
}
...
impl EdlPass {
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("EDL WGSL"),
            source: wgpu::ShaderSource::Wgsl(EDL_WGSL.into()),
        });
        ...
        // placeholder bind group (updated by set_inputs)
...
impl RgbShiftPass {
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        // CORRECTED: This layout now correctly defines 4 bindings.
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(RGBSHIFT_WGSL.into()),
        });
        let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        // CORRECTED: This BindGroup now provides 4 entries with correct binding indices, matching the layout.
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("RGBShift BG"),
            layout: &bgl,
            entries: &[
                // binding 0: tSrc (dummy color)
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&view),
                },
                // binding 1: tDepthLin (dummy depth)
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::TextureView(&view),
                },
...
impl Default for CrtUniforms {
    fn default() -> Self {
        Self { inv_size: [1.0/1280.0, 1.0/720.0], time: 0.0, intensity: 0.08, vignette: 0.35, _padding: 0.0 }
    }
}
    ...
    pub fn new(device: &wgpu::Device, dst_format: wgpu::TextureFormat) -> Self {
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            fragment: Some(wgpu::FragmentState {
                ...
                targets: &[Some(wgpu::ColorTargetState {
                    format: dst_format, blend: Some(wgpu::BlendState::REPLACE), write_mask: wgpu::ColorWrites::ALL,
                })],
                compilation_options: wgpu::PipelineCompilationOptions::default(),
            }),
            primitive: wgpu::PrimitiveState::default(),
        ...
        // dummy bind group
        let dummy = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("CRT Dummy"), size: wgpu::Extent3d { width:4, height:4, depth_or_array_layers:1 },
