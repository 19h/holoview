...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
    ...
    let c  = textureSampleLevel(tSrc,      samp, in.uv, 0.0);
    let dl = textureSampleLevel(tDepthLin, samp, in.uv, 0.0);

    // Background: z≈1 retains c
    let z = dl.r;
    if (z >= 0.9999) {
    ...
    // Label now comes from tDepthLin.g
